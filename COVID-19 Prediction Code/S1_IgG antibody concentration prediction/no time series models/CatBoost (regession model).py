...
for train_index, test_index in kf.split(X):
    CatBoost_Model = CatBoostRegressor(n_estimators=30000, learning_rate=0.008,
                                       eval_metric="MAE",
                                       objective ="MAE",
                                       random_seed=seed,
                                       loss_function='MAE',
    ...
    CatBoost_Model.fit(X[train_index],Y[train_index], eval_set=(X[test_index], Y[test_index]), verbose=True)
    part_predict = CatBoost_Model.predict(X[test_index])
