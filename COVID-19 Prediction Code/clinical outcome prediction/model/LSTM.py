...
def build_model():
    model = Sequential()
    model.add(LSTM(160, activation='relu', return_sequences=True,input_shape=(np.array(X[train_index]).shape[1],np.array(X[train_index]).shape[2])))
